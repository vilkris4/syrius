...
class ActiveSwapsListItem extends StatefulWidget {
  ...
  @override
  _ActiveSwapsListItemState createState() =>
      _ActiveSwapsListItemState();
}

class _ActiveSwapsListItemState
    extends State<ActiveSwapsListItem> {
        ...
        children: [
          ...
          Row(
              children: [
                ...
                InfoItemWidget(
                    id: "Hashlock",
                    value: FormatUtils.encodeHexString(
                        (_htlc!.hashLock)!).toString(),
                ),
                const SizedBox(
                  width: 10.0,
                ),
  ...
  Widget _getHeader(BuildContext context) {
    return Row(
        children: [
          Text(
            "${FormatUtils.formatAtomicSwapAmount(
                _htlc!.amount, _htlc!.tokenStandard)} ${token!.symbol}",
            style: Theme
                .of(context)
                .textTheme
                .headline5,
  ...
  Widget _getHtlcExpirationTime(BuildContext context) {
    ...
    "Expires in ${FormatUtils.formatExpirationTime(_remainingTime)}" : _expired;
  ...
  Widget _getAllButtons(BuildContext context) {
    return (_isActive == null) ? FutureBuilder<bool>(
  ...
  Future<bool> _getCurrentStatus() async {
    ...
    Map _status = await sl.get<ActiveSwapsWorker>().evaluateSwapStatus(
        _htlc!.id);
    /*
    _status.forEach((_swapAction, _preimage) {
  ...
  void _depositFunds(CreateHtlcBloc model) {
    ...
    model.createHtlc(
      fromAddress: _htlc!.hashLocked.toString(),
      token: token!,
      amount: "10", //_amountController.text,
  ...
  void _getPreimage() {
    if (preimage == null) {
      ...
      preimageMap.forEach((key, value) {
        if (key == (Hash.fromBytes((_htlc!.hashLock)!).toString())) {
          preimage = value;
        }
      });
       */
  ...
  Future<bool> _setVariables() async {
    ...
    _senderIsSelf =
        kDefaultAddressList.contains(_htlc!.timeLocked.toString());
    _isPending = (_htlc!.id == Hash.parse("0" * 64));
    //_depositAvailable = sl<HtlcListBloc>().getDepositAvailable(
     //   _htlc!.id, (_htlc!.hashLock)!);
    //_unlocking =
    //    sl<HtlcListBloc>().autoUnlockedSwaps.contains(_htlc!.id);
    return true;
  }
}
