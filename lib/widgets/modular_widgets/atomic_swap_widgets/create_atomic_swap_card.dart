...
import '../../reusable_widgets/dialogs/swap_dialogs/deposit_dialog.dart';
...
class _CreateAtomicSwapCardState extends State<CreateAtomicSwapCard> {
  ...
  Widget build(BuildContext context) {
    return CardScaffold(
      ...
      description:
      'Create an atomic swap on the Zenon Network of Momentum.'
      ...
      'their funds.',
      childBuilder: () => _getBalanceStreamBuilder(context),
    );
  }

  Widget _getBalanceStreamBuilder(BuildContext context) {
    return StreamBuilder<Map<String, AccountInfo>?>(
      ...
      builder: (_, snapshot) {
        ...
        if (snapshot.connectionState == ConnectionState.active) {
          if (snapshot.hasData) {
            _accountInfo = (snapshot.data![_selectedSelfAddress]!);
          ...
          child: InputField(
            ...
            validator: (value) =>
                InputValidators.correctValue(
                  value,
                  _accountInfo?.getBalanceWithDecimals(
  ...
  void _onMaxPressed() => setState(() {
    num maxBalance = (_accountInfo?.getBalanceWithDecimals)!(
      _selectedToken.tokenStandard,
    );
    _amountController.text = maxBalance.toString();
  });

  void _onGeneratePressed(String? hashType,
  ...
  void _sendCreateHtlcBlock() async {
    _createAtomicSwapButtonKey.currentState?.animateForward();
    _createHtlcBloc.createHtlc(
  ...
  Future<int> _getExpirationTime(String lockDuration) async {
    ...
    return _expirationTime + _currentTime;
  }
  ...
  void _addTokensWithBalance() {
    for (var balanceInfo in (_accountInfo?.balanceInfoList)!) {
